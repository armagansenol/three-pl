...
export class GrassMaterial {
  ...
  uniforms: { [key: string]: { value: number | boolean | THREE.Vector3 | THREE.Color | THREE.Texture } } = {
    uTime: { value: 0 },
  ...
  setupGUI(gui: GUI) {
    ...
    folder.addColor(this.grassColorProps, "baseColor").onChange((value: string) => {
      ;(this.uniforms.baseColor.value as THREE.Color).set(value)
    })
    folder.addColor(this.grassColorProps, "tipColor1").onChange((value: string) => {
      ;(this.uniforms.tipColor1.value as THREE.Color).set(value)
    })
    folder.addColor(this.grassColorProps, "tipColor2").onChange((value: string) => {
      ;(this.uniforms.tipColor2.value as THREE.Color).set(value)
    })
